...
int main( int argc, char *argv[] )
{
  if( argc < 7 )
    ...
    std::cerr << " inputImage  outputImage  multiplier iterations seedX1 seedY1 seedX2 seedY2 ..." << std::endl;
    return 1;
    }
  ...
  const double multiplier = atof( argv[3] );
  ...
  for (int i = 5; i < argc; i += 2)
    {
    index[0] = atoi( argv[i] );
    index[1] = atoi( argv[i + 1] );
    std::cout << "Adding seed : " << index << std::endl;
